...
export const ManagerDailyKpiReportsTable: React.FC<ManagerDailyKpiReportsTableProps> = ({
  managerUser,
  reports,
  employees,
  onRefreshData,
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedEmployeeId, datePreset, startDate, endDate, selectedDayOfWeek, selectedStatus, searchTerm, rowsPerPage]);
  ...
  // Filter branch reports (reports for manager's branch or supervised employees)
  const branchReports = useMemo(() => {
    return reports.filter(r => {
      if (!managerUser.branchId && !managerUser.branchName) return true;
      const sameBranchId = managerUser.branchId && r.branchId && (r.branchId === managerUser.branchId || r.branch_id === managerUser.branchId);
      const sameBranchName = managerUser.branchName && r.branchName && r.branchName.trim().toLowerCase() === managerUser.branchName.trim().toLowerCase();
      const isSupervisedEmp = employees.some(e => e.id === r.employeeId || (e.userId && e.userId === r.employeeUserId));
      return Boolean(sameBranchId || sameBranchName || isSupervisedEmp);
    });
  }, [reports, managerUser, employees]);

  // Branch Employees (non-managerial)
  const branchEmployees = useMemo(() => {
    return employees.filter(e => {
      if (e.role !== 'EMPLOYEE') return false;
      if (!managerUser.branchId && !managerUser.branchName) return true;
      return (
        (managerUser.branchId && e.branchId === managerUser.branchId) ||
        (managerUser.branchName && e.branchName?.trim().toLowerCase() === managerUser.branchName.trim().toLowerCase())
      );
    });
  }, [employees, managerUser]);
  ...
  // Calculate Branch KPI Summary Metrics
  const branchSummary = useMemo(() => {
    const calcTotals = (filterFn: (r: DailyPerformanceReport) => boolean) => {
      const subset = branchReports.filter(filterFn);
    ...
    return {
      today: calcTotals(r => (r.reportDate || r.report_date) === todayStr),
  ...
  }, [branchReports, todayStr, weekStartStr, monthStartStr, yearStartStr]);
  ...
  const filteredReports = useMemo(() => {
    ...
    if (selectedEmployeeId !== 'all') {
      list = list.filter(r =>
        ...
        r.employeeUserId === selectedEmployeeId
      );
    }
    ...
    return list;
  }, [branchReports, selectedEmployeeId, selectedStatus, datePreset, startDate, endDate, selectedDayOfWeek, searchTerm, todayStr, weekStartStr, monthStartStr, yearStartStr]);

  // Calculate Filtered Sums for Branch
  ...
  // Manager Single Action (Approve / Reject / Return)
  const handleQuickStatusChange = async (reportId: string, newStatus: 'Approved' | 'Rejected' | 'Returned') => {
    setActionLoadingId(reportId);
    try {
      await api.updateReport(reportId, { status: newStatus as any, updatedAt: new Date().toISOString() });
      onRefreshData();
    } catch (err: any) {
  ...
  const handleDelete = async (reportId: string, empName: string, date: string) => {
    if (window.confirm(`Are you sure you want to permanently delete the daily report for ${empName} on ${date}?`)) {
      try {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 1. BRANCH KPI MILESTONE SUMMARY CARDS */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            <ShieldCheck className="w-5 h-5 text-[#D4AF37]" />
            <h3 className="text-base font-extrabold text-white">
              {managerUser.branchName || 'Hamusit Branch (SOL 360)'} Daily KPI Performance Aggregation
            </h3>
          </div>
          <span className="text-xs text-[#D4AF37] font-semibold bg-[#D4AF37]/10 px-3 py-1 rounded-full border border-[#D4AF37]/30">
            {branchReports.length} Total Branch Records
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
          ...
          {/* Card 1: Today */}
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#0B4228] to-[#051F13] border border-[#D4AF37]/30 text-white shadow-lg space-y-2 hover:border-[#D4AF37]/60 transition-all">
            <div className="flex items-center justify-between text-[11px] text-gray-300 font-bold uppercase tracking-wider">
              <span>Today's Branch Output</span>
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            </div>
            <div className="text-2xl font-black text-white">
              <AnimatedCounter value={branchSummary.today.mobile + branchSummary.today.onboarding + branchSummary.today.internet + branchSummary.today.atm + branchSummary.today.merchant} suffix=" KPIs" />
            </div>
            <div className="text-[11px] text-gray-300 space-y-0.5 pt-1 border-t border-white/10">
              <div className="flex justify-between">
                <span>Mobile Banking:</span>
                <strong className="text-emerald-400"><AnimatedCounter value={branchSummary.today.mobile} /></strong>
              </div>
              <div className="flex justify-between">
                <span>Onboarding:</span>
                <strong className="text-blue-300"><AnimatedCounter value={branchSummary.today.onboarding} /></strong>
              </div>
              <div className="flex justify-between">
                <span>Merchant:</span>
                <strong className="text-amber-300"><AnimatedCounter value={branchSummary.today.merchant} /></strong>
              </div>
            </div>
          </div>
          ...
          {/* Card 2: This Week */}
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#0B4228] to-[#051F13] border border-[#D4AF37]/30 text-white shadow-lg space-y-2 hover:border-[#D4AF37]/60 transition-all">
            <div className="flex items-center justify-between text-[11px] text-[#D4AF37] font-bold uppercase tracking-wider">
              <span>This Week</span>
              <span className="text-[10px] bg-[#D4AF37]/20 px-2 py-0.5 rounded-full text-[#D4AF37]">{branchSummary.thisWeek.count} Logs</span>
            </div>
            <div className="text-2xl font-black text-[#D4AF37]">
              <AnimatedCounter value={branchSummary.thisWeek.mobile + branchSummary.thisWeek.onboarding + branchSummary.thisWeek.internet + branchSummary.thisWeek.atm + branchSummary.thisWeek.merchant} suffix=" KPIs" />
            </div>
            <div className="text-[11px] text-gray-300 space-y-0.5 pt-1 border-t border-white/10">
              <div className="flex justify-between">
                <span>Mobile Banking:</span>
                <strong className="text-emerald-400"><AnimatedCounter value={branchSummary.thisWeek.mobile} /></strong>
              </div>
              <div className="flex justify-between">
                <span>ATM Cards:</span>
                <strong className="text-violet-300"><AnimatedCounter value={branchSummary.thisWeek.atm} /></strong>
              </div>
              <div className="flex justify-between">
                <span>Merchant:</span>
                <strong className="text-amber-300"><AnimatedCounter value={branchSummary.thisWeek.merchant} /></strong>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#0B4228] to-[#051F13] border border-[#D4AF37]/30 text-white shadow-lg space-y-2 hover:border-[#D4AF37]/60 transition-all">
            <div className="flex items-center justify-between text-[11px] text-cyan-400 font-bold uppercase tracking-wider">
              ...
              <span className="text-[10px] bg-cyan-500/20 px-2 py-0.5 rounded-full text-cyan-300">{branchSummary.thisMonth.count} Logs</span>
            </div>
            <div className="text-2xl font-black text-cyan-300">
              <AnimatedCounter value={branchSummary.thisMonth.mobile + branchSummary.thisMonth.onboarding + branchSummary.thisMonth.internet + branchSummary.thisMonth.atm + branchSummary.thisMonth.merchant} suffix=" KPIs" />
            </div>
            <div className="text-[11px] text-gray-300 space-y-0.5 pt-1 border-t border-white/10">
              <div className="flex justify-between">
                <span>Mobile Banking:</span>
                <strong className="text-emerald-400"><AnimatedCounter value={branchSummary.thisMonth.mobile} /></strong>
              </div>
              <div className="flex justify-between">
                <span>Deposits:</span>
                <strong className="text-emerald-400 font-mono">ETB <AnimatedCounter value={branchSummary.thisMonth.deposits} /></strong>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#0B4228] to-[#051F13] border border-[#D4AF37]/30 text-white shadow-lg space-y-2 hover:border-[#D4AF37]/60 transition-all">
            <div className="flex items-center justify-between text-[11px] text-emerald-400 font-bold uppercase tracking-wider">
              <span>This Year</span>
              <span className="text-[10px] bg-emerald-500/20 px-2 py-0.5 rounded-full text-emerald-300">{branchSummary.thisYear.count} Logs</span>
            </div>
            <div className="text-2xl font-black text-emerald-300">
              <AnimatedCounter value={branchSummary.thisYear.mobile + branchSummary.thisYear.onboarding + branchSummary.thisYear.internet + branchSummary.thisYear.atm + branchSummary.thisYear.merchant} suffix=" KPIs" />
            </div>
            <div className="text-[11px] text-gray-300 space-y-0.5 pt-1 border-t border-white/10">
              <div className="flex justify-between">
                <span>Mobile Banking:</span>
                <strong className="text-emerald-400"><AnimatedCounter value={branchSummary.thisYear.mobile} /></strong>
              </div>
              <div className="flex justify-between">
                <span>ATM Cards:</span>
                <strong className="text-violet-300"><AnimatedCounter value={branchSummary.thisYear.atm} /></strong>
              </div>
            </div>
          </div>

          {/* Card 5: All Time */}
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#08321E] via-[#0B4228] to-[#051F13] border-2 border-[#D4AF37]/60 text-white shadow-xl space-y-2 hover:brightness-110 transition-all">
            <div className="flex items-center justify-between text-[11px] text-amber-300 font-bold uppercase tracking-wider">
              <span className="flex items-center gap-1">
                ...
                All Time Branch Total
              </span>
              <span className="text-[10px] bg-[#D4AF37] text-[#0B4228] font-extrabold px-2 py-0.5 rounded-full">Permanent</span>
            </div>
            <div className="text-2xl font-black text-[#D4AF37]">
              <AnimatedCounter value={branchSummary.allTime.mobile + branchSummary.allTime.onboarding + branchSummary.allTime.internet + branchSummary.allTime.atm + branchSummary.allTime.merchant} suffix=" Total" />
            </div>
            <div className="text-[11px] text-gray-200 space-y-0.5 pt-1 border-t border-white/10">
              <div className="flex justify-between">
                <span>Total Mobile:</span>
                <strong className="text-emerald-400"><AnimatedCounter value={branchSummary.allTime.mobile} /></strong>
              </div>
              <div className="flex justify-between">
                <span>Total Deposits:</span>
                <strong className="text-[#D4AF37] font-mono">ETB <AnimatedCounter value={branchSummary.allTime.deposits} /></strong>
              </div>
            </div>
          </div>
      ...
      {/* 2. FILTER CONTROLS (EMPLOYEE, DATES, STATUS, WEEKDAYS) */}
      <div className="p-5 rounded-3xl bg-[#08321E] border border-[#D4AF37]/40 shadow-xl text-white space-y-4">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          
          {/* Quick Date Presets */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs font-bold text-[#D4AF37] flex items-center gap-1.5 mr-1">
          ...
          <div className="relative">
            ...
            >
              ...
              <span>Export Branch Table ({filteredReports.length})</span>
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-[#08321E] border border-[#D4AF37]/40 shadow-2xl z-30 p-2 space-y-1">
                <button
                  onClick={() => {
                    downloadReportExcel(filteredReports, managerUser.branchName || 'Hamusit Branch', managerUser);
                ...
                <button
                  onClick={() => {
                    downloadReportCSV(filteredReports, managerUser.branchName || 'Hamusit Branch');
                    setShowExportMenu(false);
                  }}
                  className="w-full text-left px-3 py-2 rounded-xl text-xs text-white hover:bg-white/10 flex items-center space-x-2.5 transition-colors"
                >
                ...
                <button
                  onClick={() => {
                    printOrDownloadPDF(filteredReports, managerUser.branchName || 'Hamusit Branch', managerUser);
                    setShowExportMenu(false);
                  }}
                  className="w-full text-left px-3 py-2 rounded-xl text-xs text-white hover:bg-white/10 flex items-center space-x-2.5 transition-colors"
                >
        ...
        {/* Detailed Filters (Employee, Status, Custom Dates, Weekdays, Search) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 pt-3 border-t border-white/10">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 rounded-xl bg-black/40 border border-white/20 text-xs font-bold text-white focus:outline-none focus:border-[#D4AF37]"
            >
              <option value="all">All Supervised Staff ({branchEmployees.length})</option>
              {branchEmployees.map(emp => (
                <option key={emp.id} value={emp.id}>
                  {getUserFullName(emp)} ({emp.userId || emp.id})
                </option>
              ))}
            </select>
          </div>
          ...
          {datePreset === 'custom' ? (
            <>
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 rounded-xl bg-black/40 border border-white/20 text-xs text-white focus:outline-none focus:border-[#D4AF37]"
                />
              </div>
              <div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search staff, date, remarks..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-black/40 border border-white/20 text-xs text-white placeholder-gray-400 focus:outline-none focus:border-[#D4AF37]"
                />
              </div>
            </div>
          )}
      ...
      {/* 3. BRANCH DAILY KPI PERFORMANCE TABLE WITH FILTERED SUMS */}
      <div className="p-6 rounded-3xl bg-[#08321E] border border-[#D4AF37]/40 shadow-xl text-white space-y-4">
        
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-black text-white flex items-center gap-2">
              <Building2 className="w-5 h-5 text-[#D4AF37]" />
              Branch Employee Daily KPI Reports Register
            </h3>
            <p className="text-xs text-gray-300">
              {paginationText} (sorted newest first). Permanent storage ensures past reports are never overwritten.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 self-stretch sm:self-auto justify-between sm:justify-start">
            ...
            <div className="flex items-center space-x-2 text-xs text-gray-300">
              ...
              <select
                id="rows-per-page-select-manager"
                value={rowsPerPage}
                onChange={(e) => {
                  const val = e.target.value;
                  setRowsPerPage(val === 'all' ? 'all' : Number(val));
                }}
                className="bg-black/40 border border-white/20 text-white font-bold rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-[#D4AF37] cursor-pointer"
              >
        ...
        {filteredReports.length === 0 ? (
          <div className="p-10 text-center rounded-2xl bg-white/5 border border-white/10 text-xs text-gray-300 space-y-2">
            ...
            <p className="font-bold text-white text-sm">No Daily KPI Records Match the Selected Filter</p>
            <p className="text-gray-400 text-[11px]">
              Try choosing a different date range, employee, or status filter.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="overflow-x-auto rounded-2xl border border-white/10">
              <table className="w-full text-left text-xs text-gray-300 min-w-[950px]">
                
                {/* Sticky Header */}
                <thead className="bg-[#0B4228] text-[#D4AF37] font-bold uppercase text-[11px] sticky top-0 z-10">
                <tr>
              ...
              <tbody className="divide-y divide-white/10 bg-black/20">
                {paginatedReports.map((report) => {
                  ...
                  const empNameStr = report.employeeName || report.employee_name || 'Staff';
                  ...
                  return (
                    <tr key={report.id} className="hover:bg-white/5 transition-colors">
                      ...
                      {/* Employee */}
                      <td className="p-3.5 whitespace-nowrap">
                        <div className="font-bold text-white text-xs">{empNameStr}</div>
                        <div className="text-[10px] text-gray-400">ID: {report.employeeUserId || report.employeeId || '4994'} • SOL {report.solId || report.sol_id || '360'}</div>
                      </td>
                        ...
                        <div className="flex items-center justify-end space-x-1.5">
                          ...
                          {report.status !== 'Approved' && (
                            <button
                              ...
                              onClick={() => handleQuickStatusChange(report.id, 'Approved')}
                          ...
                          {report.status !== 'Returned' && (
                            <button
                              type="button"
                              disabled={actionLoadingId === report.id}
                              onClick={() => handleQuickStatusChange(report.id, 'Returned')}
                              title="Return for Correction"
                              className="p-1.5 rounded-lg bg-blue-500/20 hover:bg-blue-500/40 text-blue-300 transition-colors"
                            >
              ...
              {/* 4. FILTERED SUMS FOOTER */}
              <tfoot className="bg-gradient-to-r from-[#0B4228] via-[#08321E] to-[#051F13] text-white font-extrabold border-t-2 border-[#D4AF37]">
                <tr>
                  <td colSpan={3} className="p-4 text-xs font-black text-[#D4AF37] uppercase tracking-wider">
          ...
          {!isAll && totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-white/10 text-xs text-gray-300">
              <div>
                Showing <strong className="text-white">{startRecordIndex}–{endRecordIndex}</strong> of <strong className="text-[#D4AF37]">{totalRecords}</strong> records
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`min-w-[32px] h-8 flex items-center justify-center rounded-xl font-bold transition-all ${
                        currentPage === pageNum
                          ? 'bg-[#D4AF37] text-[#08321E] shadow-md font-extrabold'
                          : 'bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10'
                      }`}
                    >
                      {pageNum}
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#08321E] border border-[#D4AF37]/40 rounded-3xl p-6 w-full max-w-lg text-white space-y-4 shadow-2xl">
            ...
            {viewingReport.managerComment && (
              <div className="p-3 bg-black/30 border border-[#D4AF37]/30 rounded-xl text-xs">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/10">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-[#D4AF37] font-bold text-xs flex items-center gap-1.5"
              >
                <Edit className="w-3.5 h-3.5" />
                <span>Edit KPI Record</span>
              </button>

              <button
                onClick={() => setViewingReport(null)}
                className="px-5 py-2 rounded-xl bg-[#D4AF37] text-[#0B4228] font-bold text-xs hover:bg-[#e0be4d]"
              >
                Close
              </button>
            </div>
